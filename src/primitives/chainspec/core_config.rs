...
use casper_types::system::auction::VESTING_SCHEDULE_LENGTH_MILLIS;
...
pub struct CoreConfig {
    ...
    /// Protocol version from which nodes are required to hold strict finality
    /// signatures.
    ...
    /// Used to determine finality sufficiency for new joiners syncing blocks
    /// created in a protocol version before
    ...
    /// If you bond with a sufficient bid in era N, you will be a validator in
    /// era N + auction_delay + 1
    pub auction_delay: u64,

    /// The period after genesis during which a genesis validator's bid is
    /// locked.
    pub locked_funds_period: TimeDiff,

    /// The period in which genesis validator's bid is released over time after
    /// it's unlocked.
    ...
    /// Global state prune batch size (0 means the feature is off in the current
    /// protocol version).
    ...
    /// Auction entrypoints such as "add_bid" or "delegate" are disabled if this
    /// flag is set to `false`. Setting up this option makes sense only for
    /// private chains where validator set rotation is unnecessary.
    pub(crate) allow_auction_bids: bool,
    /// Allows unrestricted transfers between users.
    pub(crate) allow_unrestricted_transfers: bool,
    /// If set to false then consensus doesn't compute rewards and always uses
    /// 0.
...
impl CoreConfig {
    /// The number of eras that have already started and whose validators are
    /// still bonded.
    pub fn recent_era_count(&self) -> u64 {
        // Safe to use naked `-` operation assuming `CoreConfig::is_valid()` has been
        // checked.
        self.unbonding_delay - self.auction_delay
    }

    /// Returns `false` if unbonding delay is not greater than auction delay to
    /// ensure that `recent_era_count()` yields a value of at least 1.
    pub fn is_valid(&self) -> bool {
        ...
        // If the era duration is set to zero, we will treat it as explicitly stating
        // that eras should be defined by height only.  Warn only.
...
impl CoreConfig {
    ...
    pub fn random(rng: &mut TestRng) -> Self {
        ...
        let administrators =
            (0..rng.gen_range(0..=10u32)).map(|_| PublicKey::random(rng)).collect();
        let refund_handling = {
...
pub enum RefundHandling {
    /// Refund of excess payment amount goes to either a pre-defined purse, or
    /// back to the sender and the rest of the payment amount goes to the
    /// block proposer.
    Refund {
        /// Computes how much refund goes back to the user after deducting gas
        /// spent from the paid amount.
        ///
        /// user_part = (payment_amount - gas_spent_amount) * refund_ratio
        /// validator_part = payment_amount - user_part
        ///
        /// Any dust amount that was a result of multiplying by refund_ratio
        /// goes back to user.
        refund_ratio: Ratio<u64>,
    },
    /// Burns the refund amount.
    Burn {
        /// Computes how much of the refund amount is burned after deducting gas
        /// spent from the paid amount.
        refund_ratio: Ratio<u64>,
    },
}
...
pub enum FeeHandling {
    ...
    PayToProposer,
    /// Transaction fees are accumulated in a special purse and then distributed
    /// during end of era processing evenly among all administrator
    /// accounts.
    ///
